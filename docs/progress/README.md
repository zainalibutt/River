...
scripts. Their output is intentionally kept under the ignored
`art/out/proofs/native-gold/` directory; generated renders are not shipped as
repository assets.

The proof sequence now covers:

- the isolated glamorous female source in front, three-quarter and profile;
- the planted seated posture with table and hand contact;
- native skin, patterned red dress, brown eyes, brows, lashes and controlled
  ponytail under the same source recipe;
- blink, soft-smile and frustration expression states after GLB export; and
- a one-seat Rooftop Chrome review using the actual served GLB.
