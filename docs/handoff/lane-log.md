...
| 2026-08-27 | Camera to the reference | Claude | `c490b84` | 3.2m out and 1.5m up against 6.1m and 4.05m. The terrace is 4.0m across, so the old camera stood **outside the parapet looking in**, and from 4.05m you saw into the open tops of the seat cylinders |
...
| 2026-08-27 | Seat occupancy | Claude | `5657e91` | The venue bakes nine characters in as geometry, so every seat showed a person whatever the room said. Lint caught the wiring half-done - the prop was accepted and never passed on |
| 2026-08-27 | Hold-to-read plates | Claude | `44e45d6` | Nine permanent nameplates replaced with hold-to-show. Also fixed a real leak: a key held when the window loses focus never sends its keyup, so alt-tab left **the card peek up, showing your hand to the room** |
| 2026-08-27 | Repository hygiene | Claude | `5ffea23`, `868e57c` | 53 mentions of the reference game cleaned from 14 files, reference material untracked, and `hygiene.test.ts` added to gate it. The gate caught two 'clone' phrasings a name search had missed |
